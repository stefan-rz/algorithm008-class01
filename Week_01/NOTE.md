...
|                                                              |          |                                                              |                                                              |             |                            | 五毒神掌 |        |        |        |        |
  ...
  | [977. 有序数组的平方](https://leetcode-cn.com/problems/squares-of-a-sorted-array/) | 数组     | 相向双指针<br />tricky的做法（python独占)：直解计算平方值，然后sort，时间复杂度还是O(N) | 两左右指针，左指针指向的是负整数，右指针指向的是正整数<br /><br />循环出口：左指针指向了正整数， 或者右指针指向了负整数<br />判断条件是当左指针指向的当前负整数的平方大于右指针指向的当前正整数的平方，则把右指针指向平方指放入结果集，右子针往左移动，反之则左指针往右移动<br />根据循环跳出条件，我们需要后处理去处理那些没有来得及处理的整数，有点类似于merge 两个sorted 数组的后处理。 | O(N)        | O(N) - 开辟了N长度的结果集 | 刷       | 已刷   | 已刷   | 一周后 | 面试前 |
